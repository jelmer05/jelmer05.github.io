...
function A(o, e, t) {
  ...
  const l = async () => {
    ...
    if (h)
      try {
        const f = await o(...h.args);
        h.resolve(f);
      } catch (f) {
        h.reject(f);
      }
    const u = setTimeout(() => {
      i--, r.length > 0 && l(), s = s.filter((f) => f !== u);
    }, t);
  ...
  return a.abort = () => {
    n = !0, s.forEach(clearTimeout), s = [], r.forEach(
      (h) => h.reject(() => new x("Throttle function aborted"))
    ), r.length = 0;
  }, a;
}
class m {
...
const P = function(o, e) {
  ...
  const e = new m().escapeHTML, t = { ...o.attrs }, { linktype: r = "url" } = o.attrs;
...
class ae {
  ...
  constructor(e, t) {
    ...
    let r = e.endpoint || t;
    if (!r) {
      const n = new m().getRegionURL, l = e.https === !1 ? "http" : "https";
    ...
    s.set("Content-Type", "application/json"), s.set("Accept", "application/json"), e.headers && (e.headers.constructor.name === "Headers" ? e.headers.entries().toArray() : Object.entries(e.headers)).forEach(([l, a]) => {
      s.set(l, a);
    }), s.has($) || (s.set($, R.defaultAgentName), s.set(
      R.defaultAgentVersion,
      R.packageVersion
    ));
    let i = 5;
    e.oauthToken && (s.set("Authorization", e.oauthToken), i = 3), e.rateLimit && (i = e.rateLimit), e.richTextSchema ? this.richTextResolver = new T(e.richTextSchema) : this.richTextResolver = new T(), e.componentResolver && this.setComponentResolver(e.componentResolver), this.maxRetries = e.maxRetries || 10, this.retriesDelay = 300, this.throttle = A(
  ...
  async getAll(e, t, r, s) {
    const i = (t == null ? void 0 : t.per_page) || 25, n = `/${e}`.replace(/\/$/, ""), l = r ?? n.substring(n.lastIndexOf("/") + 1), a = 1, h = await this.makeRequest(
      n,
      t,
      i,
      a,
    ...
    return this.helpers.flatMap([h, ...f], (g) => Object.values(g.data[l]));
  }
  post(e, t, r) {
    const s = `/${e}`;
  ...
  _insertRelations(e, t, r, s) {
    r.includes(`${e.component}.${t}`) && (typeof e[t] == "string" ? e[t] = this.getStoryReference(s, e[t]) : Array.isArray(e[t]) && (e[t] = e[t].map((i) => this.getStoryReference(s, i)).filter(Boolean)));
  }
  iterateTree(e, t, r) {
    const s = (i) => {
      if (i != null) {
  ...
  async cacheResponse(e, t, r, s) {
    ...
    return new Promise(async (l, a) => {
      ...
      try {
        ...
        return t.version === "published" && e !== "/cdn/spaces/me" && await n.set(i, f), f.data.cv && t.token && p[t.token] !== f.data.cv && (await this.flushCache(), p[t.token] = f.data.cv), l(f);
      } catch (u) {
