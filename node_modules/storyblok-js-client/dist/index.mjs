  ...
  optimizeImages(e, t) {
    ...
    return e = e.replace(
      ...
      if (c && c.length > 0) {
        ...
        let d = "";
        return p.srcset && (d += `srcset="${p.srcset}" `), p.sizes && (d += `sizes="${p.sizes}" `), a.replace(/<img/g, `<img ${d.trim()}`);
      }
      return a;
    })), e;
  }
...
class ie {
  ...
  constructor(e, t) {
    ...
    let i = 5;
    e.oauthToken && (r.set("Authorization", e.oauthToken), i = 3), e.rateLimit && (i = e.rateLimit), e.richTextSchema ? this.richTextResolver = new T(e.richTextSchema) : this.richTextResolver = new T(), e.componentResolver && this.setComponentResolver(e.componentResolver), this.maxRetries = e.maxRetries || 5, this.throttle = x(this.throttledRequest, i, 1e3), this.accessToken = e.accessToken || "", this.relations = {}, this.links = {}, this.cache = e.cache || { clear: "manual" }, this.helpers = new k(), this.resolveCounter = 0, this.resolveNestedRelations = e.resolveNestedRelations || !0, this.stringifiedStoriesCache = {}, this.client = new se({
      baseURL: s,
      timeout: e.timeout || 0,
      headers: r,
  ...
  async getAll(e, t, s, r) {
    ...
    const p = await this.helpers.asyncMap(
  ...
  async cacheResponse(e, t, s) {
    ...
    return new Promise(async (n, l) => {
      ...
      try {
        ...
        return t.version === "published" && e != "/cdn/spaces/me" && await i.set(r, u), u.data.cv && t.token && (t.version === "draft" && y[t.token] != u.data.cv && await this.flushCache(), y[t.token] = t.cv ? t.cv : u.data.cv), n(u);
      } catch (c) {
