	...
	<section
		...
		<div
			...
			<div class="flex w-3/5 flex-col justify-between gap-4 self-start">
				<router-link
					to="/"
					class="flex items-center gap-2 hover:animate-pulse"
					><img
			...
			<div class="flex justify-center">
				<video
					v-if="blok.mainBanner.filename.includes('.mp4')"
				...
				<img
					v-else
					:src="blok.mainBanner.filename"
					:alt="blok.mainBanner.alt"
					class="w-10/12 self-center rounded-lg sm:w-6/12" />
			</div>
		</div>
		...
		<div
			...
			<div class="flex w-full justify-evenly">
				<div
					v-for="techniek in blok.technieken"
					class="relative">
					<h1 class="font-medium">{{ techniek }}</h1>
					<div
						class="frame absolute left-0 top-3/4 z-[-1] w-full bg-orange-theme py-1 opacity-80"></div>
				</div>
			</div>
		</div>
		<main class="container mx-auto">
			...
			<div
				class="text p-2 leading-7"
				v-for="content in blok.mainText.content">
				<p>
					<span
						:class="{
